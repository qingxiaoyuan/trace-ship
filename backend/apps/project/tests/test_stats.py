...
def project(manager):
    ...
    for name in ("repo-a", "repo-b"):
        repo = Repository.objects.create(repo_type="git", name=name, url=f"https://{name}")
        ProjectComponent.objects.create(
            project=project, repository=repo, component_code=name, display_name=name,
        )
    return project
...
def test_detail_returns_all_counts(client, project):
    ...
    ReleaseRecord.objects.create(
        project=project,
        repository=project.project_components.first().repository,
...
def test_list_search_matches_code_or_name(client, project):
    ...
    missed = client.get("/api/projects/", {"search": "不存在的项目"})
