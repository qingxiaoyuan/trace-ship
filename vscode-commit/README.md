...
| **合并冲突解决**       | 支持内联 CodeLens 模式与 VS Code 原生合并编辑器模式，可一键切换       |
| **暂存区刷新**         | 「暂存的更改」分组标题右侧提供刷新按钮，手动刷新状态                   |
...
| `commit.conflictResolutionMode` | 否   | 冲突解决模式：`inline`（默认，内联 CodeLens）/ `mergeEditor`（原生合并编辑器） |
...
当仓库存在合并冲突时，侧边栏会显示「冲突」分组，并支持两种解决模式：

- **内联 CodeLens 模式（默认）**：点击冲突文件会在普通编辑器中打开，文件中的冲突标记（`<<<<<<<` / `=======` / `>>>>>>>`）行会显示快捷按钮：
  - **接受当前更改**：保留当前分支内容
  - **接受传入的更改**：保留传入分支内容
  - **接受两个更改**：同时保留两边内容
  - **比较变更**：打开 diff 视图对比两边差异
  - 文件行右侧的 **⇄** 按钮可随时打开 VS Code 原生合并编辑器。
- **原生合并编辑器模式**：点击冲突文件直接打开 VS Code 三栏合并编辑器；此模式下扩展的 CodeLens 会自动隐藏，避免与原生 UI 重复。

切换方式：

- 命令面板运行 **合并冲突：切换冲突解决模式**。
- 或直接在设置中修改 `commit.conflictResolutionMode`（`inline` / `mergeEditor`）。

### 7. 手动刷新状态

- 仓库标题右侧的 **↻** 按钮可刷新整个仓库的变更状态。
- 「暂存的更改」分组标题右侧也提供 **↻** 按钮，方便单独刷新暂存区状态。
...
- **AI 上下文**：显示暂存区 diff 的字符数，帮助你判断提交内容是否过长。数值过大会导致 AI 生成变慢或超时：
  - 蓝色：正常；
  - 黄色：超过 8,000 字符，建议精简暂存文件；
  - 红色：超过 20,000 字符，建议拆分为多次提交。
...
第四个 chip「AI 上下文」，并带阈值颜色提示：
蓝色：正常；
黄色：> 8,000 字符，建议精简暂存文件；
红色：> 20,000 字符，建议拆分提交。
...
5. 提交的文件较多时，AI 生成可能需要较长时间。若生成过程中想中断，可点击右上角的「⋯」更多操作按钮，选择「停止生成」。
...
插件运行过程中产生的日志（如 Git 监听异常、打开 diff 失败、AI 接口报错等）可以通过以下方式查看：

1. **已安装插件运行时**
   - 打开 VS Code **输出**面板（快捷键 `Ctrl+Shift+U` / `Cmd+Shift+U`）。
   - 在右上角下拉框中选择 **扩展宿主**（Extension Host）。
   - 带有 `[SidebarProvider]`、`[openFileInDiffView]` 等前缀的即为本插件日志。

2. **本地 F5 调试扩展时**
   - 日志会输出到左侧的 **调试控制台**（Debug Console）。
   - 同时也可以在输出面板的 **扩展宿主** 中查看。

3. **查看更底层日志**
   - 打开 **帮助 → 切换开发人员工具**（`Ctrl+Shift+I` / `Cmd+Shift+I`）。
   - 在 Console 标签页中可查看 Webview 与扩展进程的所有输出。

> 提示：如果遇到「AI 生成 Commit」无响应或报错，优先查看输出面板的扩展宿主日志，通常能找到 API 请求失败或 Git 异常的原因。
