...
export function PackageNodeCard() {
  ...
  return (
    <div className="tech-card rounded-xl px-5 py-4">
      ...
      <div className="mt-4 overflow-hidden rounded-lg border border-indigo-50 max-md:space-y-3 max-md:overflow-visible max-md:rounded-none max-md:border-0">
        ...
        {isLoading ? (
          ...
          nodes.map((node) => (
            <div
              key={node.id}
              className="border-b border-indigo-50 last:border-0 max-md:rounded-xl max-md:border max-md:border-indigo-100/70 max-md:bg-white max-md:p-4"
            >
              {/* 桌面端网格行 */}
              <div className="hidden grid-cols-12 items-center gap-3 px-4 py-2.5 text-[13px] md:grid">
                ...
                <div className="col-span-1 text-[12px] text-slate-600 md:col-span-2">
                  {node.credential_name || '-'}
                </div>
                <div className="col-span-1 truncate font-mono text-[12px] text-slate-500 md:col-span-2" title={node.work_root}>
                  {node.work_root}
                </div>
              ...
              <div className="md:hidden">
                ...
                <div className="mt-1.5 flex items-center gap-1.5 text-[11px] text-slate-400">
                  ...
                  <span className="truncate">{node.credential_name || '-'}</span>
                </div>
                <div className="mt-1 flex items-center gap-1.5 text-[11px] text-slate-400">
                  <Folder className="h-3.5 w-3.5 shrink-0" strokeWidth={1.5} />
                  <span className="truncate font-mono">{node.work_root}</span>
                </div>
                <div className="mt-3 flex items-center justify-between border-t border-indigo-50 pt-3">
                  <span className="min-w-0 truncate text-[11px] text-slate-400">{node.description || ''}</span>
                  <div className="flex shrink-0 items-center gap-1">
                    <button
                      ...
                      disabled={testMutation.isPending}
                      onClick={() => testMutation.mutate(node.id)}
                      className="rounded-md p-2 text-slate-400 transition-colors hover:bg-indigo-50 hover:text-indigo-600 disabled:opacity-60"
                    >
