...
const sections: GuideSection[] = [
  {
    ...
    intro: 'Trace Ship 以「项目关联仓库」组织发布管理：项目用于成员、权限、审批和构建配置；仓库是软件与版本的最小单位。第一次使用可按以下步骤完成一次发布。',
    steps: [
      { title: '创建项目', desc: '在「项目/产品」页新建项目，设置项目负责人和成员。' },
      { title: '登记仓库', desc: '在「仓库」页登记 GitLab 软件仓库。登记仓库不要求绑定项目；同一仓库只需登记一次。系统会为该仓库生成正式 / RC / Beta 三套发布审批流程，仓库创建者可在仓库详情的「审批流」中编辑。' },
      { title: '关联仓库', desc: '在项目详情的「软件仓库」页关联已有仓库，并设置默认分支和源码子目录等当前项目设置。' },
      { title: '准备凭证与打包（可选）', desc: '在「凭证」页维护 GitLab Token、SVN 账号等凭证，并按仓库、项目与操作范围配置借用授权；需要构建时在关联仓库下维护打包配置。' },
      { title: '发起仓库发布', desc: '在「新建发布」页选择项目和一个已关联仓库，再选择分支。系统按仓库 Tag 与版本规则计算版本，审批通过后只为该仓库推送一个 Tag。' },
    ],
  },
  {
    id: 'release-flow',
    ...
    steps: [
      { title: '创建发布申请', desc: '选择项目、一个已关联仓库与目标分支；未手动填写版本号时，系统会基于该仓库已有 Tag 和仓库版本规则自动计算。' },
      ...
      { title: '自动打包', desc: '推 Tag 成功后，仅本次项目与仓库关联下开启「发布后自动打包」的配置会创建打包任务；打包异常不影响发布状态。' },
    ],
  ...
  },
  {
    ...
    intro: '项目成员分为开发、测试、项目管理员、审核人、只读等角色，系统管理员负责平台级配置。',
    bullets: [
      '开发人员：关注「提交审查」中自己提交的合规情况，按提交规范书写 Commit Message；需要发布时向项目管理员提出申请。',
      '测试人员：在发布详情中查看本次仓库发布的变更清单与关联提交，验证发布后的软件行为。',
      '项目管理员：维护项目信息、成员、关联仓库与打包配置，发起发布申请并跟进审批进度。仓库审批流由仓库创建者在仓库详情中维护。',
  ...
  },
  {
    ...
    steps: [
      ...
      { title: '仓库打包配置', desc: '项目管理员在项目详情的「打包配置」中选择软件仓库与镜像（按坐标自动建档），配置构建目录、产物目录、环境变量，可选自定义脚本与 SVN 推送。' },
      { title: '执行打包', desc: '在「打包看板」手动触发或由发布自动触发。系统会克隆源码到工作区，仅挂载源码、产物、临时目录进容器执行打包。手动触发不推送 SVN；仅正式发布且开启自动打包的任务才会推送 SVN。' },
  ...
  },
  {
    ...
    bullets: [
      ...
      '发布后没有自动打包？检查本次项目与仓库对应的打包配置是否开启「发布后自动打包」，并确认镜像与凭证配置正确。',
...
const moduleEntries = [
  { icon: FolderKanban, name: '项目/产品', path: '/projects', desc: '组织成员、关联仓库与发布' },
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-[13px] text-slate-500">
            从创建项目、关联仓库到完成发布的完整操作指引，按角色快速找到适合自己的使用路径
          </p>
        </div>
