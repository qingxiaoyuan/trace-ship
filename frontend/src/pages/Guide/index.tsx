...
const sections: GuideSection[] = [
  {
    ...
    intro: 'Trace Ship 以「项目」为核心组织发布管理。第一次使用时，按以下 5 步即可完成一次完整发布。',
    steps: [
      { title: '创建项目', desc: '在「项目」页新建项目，配置版本号规则与分支规则，并添加项目成员。项目创建后系统会自动生成内置的发布审批流程。' },
      { title: '接入仓库', desc: '在项目详情或「仓库」页添加 GitLab 仓库，绑定访问凭证后系统开始同步提交记录。' },
      { title: '准备凭证', desc: '在「凭证」页录入 GitLab Token、SVN 账号等凭证，凭证加密存储，接口返回时自动脱敏。' },
      { title: '配置打包（可选）', desc: '在项目详情的「打包配置」中选择打包镜像、构建目录与产物目录，需要时可开启「发布后自动打包」。' },
      { title: '发起发布', desc: '在「新建发布」页选择项目与分支，系统自动计算版本号并预览变更，填写发布说明后提交审批，审批通过即自动推 Tag 完成发布。' },
    ],
  },
  {
    id: 'release-flow',
    ...
    steps: [
      { title: '创建发布申请', desc: '选择项目、仓库与目标分支；未手动填写版本号时，系统会基于仓库已有 Tag 和项目版本规则自动计算。' },
      ...
      { title: '提交审批', desc: '按发布类型匹配启用的审批流程，创建审批实例，发布进入「待审批」状态。' },
      ...
      { title: '自动打包', desc: '推 Tag 成功后，开启「发布后自动打包」的项目会自动创建打包任务；打包异常不影响发布状态。' },
    ],
  ...
  },
  {
    ...
    intro: '项目成员分为开发、测试、项目经理、审批人、只读五种角色，系统管理员负责平台级配置。',
    bullets: [
      '开发人员：关注「提交审查」中自己提交的合规情况，按提交规范书写 Commit Message；需要发布时向项目经理提出申请。',
      '测试人员：在发布详情中查看本次发布的变更清单与关联提交，验证发布后可在项目内反馈。',
      '项目经理：维护项目信息、成员、仓库与打包配置，发起发布申请并跟进审批进度。',
  ...
  },
  {
    ...
    steps: [
      ...
      { title: '项目打包配置', desc: '项目管理员在项目详情中直接选择镜像（按坐标自动建档），配置构建目录、产物目录、环境变量，可选自定义脚本与 SVN 推送。' },
  ...
  },
  {
    ...
    bullets: [
      ...
      '「非法提交预警」集中展示需要立即处理的提交，点击进入详情可查看违规原因。',
      '提交信息建议遵循约定式提交格式（如 feat: / fix: 前缀），具体规范以项目配置为准。',
  ...
  },
  {
    ...
    bullets: [
      ...
      '仓库绑定、打包推送 SVN 等操作会自动解析并复用已录入的凭证。',
  ...
  },
  {
    ...
    bullets: [
      ...
      '版本号不符合预期？检查项目的版本号规则配置；也可以创建发布时手动填写版本号覆盖自动计算结果。',
      '提交审批按钮不可点？确认发布处于「草稿」状态且发布说明非空。',
      '发布后没有自动打包？检查项目打包配置是否开启「发布后自动打包」，并确认镜像与凭证配置正确。',
...
const moduleEntries = [
  { icon: FolderKanban, name: '项目', path: '/projects', desc: '组织成员、仓库与发布' },
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-[13px] text-slate-500">
            从创建项目到完成发布的完整操作指引，按角色快速找到适合自己的使用路径
          </p>
        </div>
