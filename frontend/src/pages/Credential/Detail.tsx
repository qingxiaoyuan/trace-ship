...
const usageIconMap: Record<string, React.ComponentType<{ className?: string; strokeWidth?: number }>> = {
  repository: GitFork,
  release: Tag,
  jenkins: GitBranch,
  default: GitCompare,
...
> = {
  success: { border: 'border-emerald-200', bg: 'bg-emerald-50', text: 'text-emerald-700', label: '成功' },
  failure: { border: 'border-rose-200', bg: 'bg-rose-50', text: 'text-rose-600', label: '失败' },
};
...
export default function CredentialDetail() {
  ...
  const status = getCredentialStatus(credential.is_active, credential.expires_at);
  const subText =
      ...
      <div className="tech-card rounded-xl p-5">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="mt-1 flex flex-wrap items-center gap-2 text-[12px] text-slate-500">
                <span className="rounded bg-slate-100 px-1 py-0.5 font-mono">{credential.cred_type}</span>
        ...
        <div className="mt-5 grid grid-cols-2 gap-3 border-t border-indigo-50 pt-4 md:grid-cols-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[13px] font-semibold text-slate-900">{relatedCount} 个</span>
          </div>
        </div>
      </div>
        ...
        <div className="p-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div className="space-y-3">
                {[
                  ...
                  { label: '凭证类型', value: credentialTypeMap[credential.cred_type] },
                  { label: '认证方式', value: credential.auth_mode || '-' },
                  {
                    label: '作用域',
                    value: `${credentialScopeMap[credential.scope]}（${credential.scope}）`,
                  },
                  { label: '创建时间', value: formatDate(credential.created_at) },
                  { label: '过期时间', value: formatDate(credential.expires_at, '永久有效') },
                ].map((item) => (
                  <div
                    key={item.label}
                  ...
                  >
                    ...
                    <span className="text-[13px] text-slate-800">{item.value}</span>
                  </div>
                ))}
              </div>
              ...
              <div className="space-y-4">
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="text-[12px] font-medium text-slate-600">脱敏数据</span>
                    <button
                      type="button"
                      onClick={() => message.info('明文需二次验证后查看，当前仅展示脱敏数据')}
                ...
                <div className="rounded-lg border border-emerald-200 bg-emerald-50/40 p-3">
                  ...
                  <p className="text-[11px] text-slate-500">
                    凭证使用 AES-256 加密存储，接口仅返回脱敏数据，明文需二次验证后查看。
                  </p>
                </div>

                <div>
                  <div className="text-[12px] font-medium text-slate-600 mb-1.5">权限范围</div>
                  <div className="flex flex-wrap gap-1.5">
                    ...
                    {credential.cred_type !== 'gitlab_token' && (
                      <span className="text-[12px] text-slate-400">由具体凭证类型决定</span>
                    )}
                  </div>
                </div>
              </div>
          ...
          {activeTab === 'usage' && (
            <div className="divide-y divide-indigo-50/50 border border-indigo-100 rounded-lg">
              {usageLoading ? (
                <div className="px-4 py-8 text-center text-[13px] text-slate-400">加载中…</div>
              ) : usageRecords.length === 0 ? (
                <div className="px-4 py-8 text-center text-[13px] text-slate-400">暂无使用记录</div>
              ) : (
                usageRecords.map((record) => {
                  const Icon = usageIconMap[record.module] || usageIconMap.default;
                  const resultCfg = usageResultConfig[record.result || 'success'];
                  return (
                    <div
                      key={record.id}
              ...
              {credential.scope === 'project' && credential.project_id ? (
                <div className="rounded-lg border border-indigo-100 bg-white p-3 flex items-center gap-3 tech-card-hover transition-colors">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-[11px] text-slate-400">项目 · 凭证作用域：项目</div>
                  </div>
                </div>
              ) : (
