...
export const changelogEntries: ChangelogEntry[] = [
  {
    ...
    items: [
      { category: 'feature', text: '远程打包节点新增「芯片架构」配置（x86 32/64 位、ARM 32/64 位，默认 x86 64 位）：节点列表名称旁展示架构徽标，打包配置选择节点时下拉同步显示架构，便于匹配构建机' },
      { category: 'fix', text: '修复远程节点为麒麟 / Linux 实体机时打包脚本仍按 Windows bat 处理的问题：脚本编辑器语言、文件名（pack-custom.sh）与 AI 生成脚本现在按所选节点操作系统决定，麒麟节点一律使用 sh 脚本' },
      { category: 'feature', text: '产品成员授权分级收缩：产品负责人可授予全部角色；软件管理员不能授予产品负责人 / 软件管理员；开发、测试等其他角色也能拉人进产品，但只能授予开发 / 测试角色；移除成员与调整角色仍仅产品管理员可操作' },
      { category: 'improvement', text: '提交审查「已发布回溯」列表的审查一栏新增蓝色「待复核·n」徽标：无须整改显示「正常」，待整改与待复核数量可同时展示，一眼看清整改闭环进度' },
      { category: 'feature', text: '系统管理新增「通知发送」页面：管理员可向全部用户或指定用户发送系统通知，接收人在通知中心「系统」分类查看；发送权限为独立的 system.notification 编码，可在角色管理中按需授予' },
    ],
  },
  {
    version: '2026.08.27',
    date: '2026-08-27',
    summary: '工作台全面改版：全区块可点击跳转、真实待办与快捷审批；修复打包配置历史记录与审批人显示问题',
    items: [
      { category: 'improvement', text: '工作台全面改版：KPI 卡、发布流水线状态列、最近发布列表行、待办事项均可点击跳转到对应模块，问候语中的待办/异常数字也可直接点击，消除全部"点了没反应"的假交互' },
      { category: 'feature', text: '「我的待办」接入真实待办数据：待我审批的发布支持在列表内直接「通过 / 驳回」（驳回可填原因），失败打包带「查看日志」、运行中打包带「查看进度」快捷入口' },
      { category: 'improvement', text: '工作台新增「发布趋势」图（后端按天聚合的成功/失败数据，替代原打包任务小样本统计）；打包成功率卡片支持点击跳转打包看板' },
      { category: 'feature', text: '「导出周报」功能落地：一键导出近 7 天发布记录 CSV（版本/产品/仓库/类型/状态/发布人/时间）' },
      { category: 'improvement', text: '审批详情「发布摘要」新增「所属软件」字段，审批头部产品名加粗加深，一眼可辨是哪个软件要发布' },
      { category: 'fix', text: '修复打包配置详情页误显示「该配置暂无打包记录」：看板任务列表拆分进行中/历史后，配置最新任务查找仍只查进行中列表，已结束任务永远找不到，现改为按配置拉取完整记录后自动选中最新一条' },
      { category: 'fix', text: '修复审批流程时间线只显示第一个审批人的问题：会签/或签节点的全部审批人现在完整展示；未激活节点也会显示配置的审批人（产品负责人/角色/指定用户），不再一律显示「待分配审批人」' },
      { category: 'fix', text: '修复工作台发布趋势接口报错（PostgreSQL 下 created_at 字段歧义）及修复后趋势数据全为 0 的日期匹配问题' },
    ],
  },
  {
    version: '2026.08.21',
    date: '2026-08-21',
    summary: '打包工作区自动清理防磁盘占满，打包看板拆分进行中/历史，推 tag 失败支持一键重试',
    items: [
      { category: 'feature', text: '打包任务结束（成功/失败/取消）自动删除工作区源码与临时文件，构建产物与日志保留，避免历史源码长期占用磁盘' },
      { category: 'feature', text: '系统配置页新增「打包工作区清理」卡片：可配置打包产物保留天数（默认 30 天），每天 8:00 自动清理超期产物（仅删产物文件，构建日志保留）' },
      { category: 'feature', text: '远程 Windows 打包节点每天 0:00 自动清理残留的打包目录（含勾选「调试保留目录」的任务），正在运行的任务目录自动跳过；节点工作目录误配置为盘符根目录时拒绝清理，防误删' },
      { category: 'improvement', text: '远程打包任务失败或取消时自动回收节点上的任务目录（此前仅成功时清理，失败残留会一直占用节点磁盘）' },
      { category: 'improvement', text: '打包看板构建列表拆分为「进行中 / 打包历史」两个 Tab：进入默认只显示进行中的任务（每 5 秒自动刷新），历史任务切换到「打包历史」查看' },
      { category: 'feature', text: '推 tag 失败的发布支持一键重试：发布详情页新增「重试推 tag」按钮（仅审批已通过、推 tag 环节失败的发布显示），重试成功直接发布完成，无需重新走审批流程' },
    ],
  },
  {
    version: '2026.08.20',
    ...
    items: [
      ...
      { category: 'improvement', text: '仓库/产品详情的发布版本列表中，正式版徽标由黑色调整为靛蓝色，与移动端配色一致' },
  ...
  },
  {
    ...
    items: [
      ...
      { category: 'feature', text: '仓库详情「标签」支持删除：点击标签卡片删除按钮后需输入完整 Tag 名称二次确认，删除远程 Tag 并同步清理本地缓存（不可恢复，仅产品管理员可操作；远程 Tag 已不存在时幂等成功）' },
  ...
  },
  {
    version: '2026.08.17',
    date: '2026-08-17',
    summary: '打包看板支持产品过滤与本地记忆、支持按仓库分支最新代码直打包',
    items: [
      ...
      { category: 'improvement', text: '打包看板的构建列表与打包配置列表支持按产品过滤（默认全部产品），选择过的产品写入本地缓存，下次进入自动复用' },
  ...
  },
  {
    ...
    items: [
      { category: 'feature', text: '已发布版本支持删除：发布详情页对「已发布」状态提供「删除版本」入口，二次确认需输入完整 Tag 名称后执行，同时删除远端 Tag、发布记录及关联打包任务（操作不可恢复，仅产品管理员可操作）' },
      { category: 'feature', text: '发布草稿与已驳回记录支持删除：详情页提供删除入口并二次确认；草稿可由创建人或产品管理员删除，已驳回记录需产品管理员' },
      ...
      { category: 'feature', text: '生成时可参考平台内历史打包成功的脚本（同产品优先、跨产品兜底，按相关度与成功次数排序），默认开启可随时关闭' },
  ...
  },
  {
    ...
    items: [
      ...
      { category: 'feature', text: '产品列表返回当前用户有效角色（my_role），打包配置产品下拉仅列出可管理的产品' },
      ...
      { category: 'improvement', text: '仓库列表优先显示完整克隆地址，编辑仓库时回填全地址；产品详情仓库表格调整列宽（地址列加宽、操作栏加宽、同步时间不换行）' },
  ...
  },
  {
    ...
    items: [
      { category: 'feature', text: '打包看板构建列表支持分页（每页 15 条）、关键字搜索与按产品过滤' },
      ...
      { category: 'improvement', text: '打包配置参数仅产品管理员 / 软件管理员可维护，其他角色在看板中隐藏编辑入口并显示锁定提示' },
      { category: 'improvement', text: '新建打包配置的产品下拉只列出当前用户可管理（产品管理员 / 软件管理员）的产品' },
      { category: 'improvement', text: '成员记录中的软件管理员角色优先生效，产品负责人被显式设为软件管理员后自动获得对应权限' },
      { category: 'fix', text: '补充产品可见性回归保障：非产品成员无法在产品列表看到该产品，访问详情返回 404' },
  ...
  },
  {
    ...
    items: [
      ...
      { category: 'improvement', text: '重构产品角色权限模型，统一产品资源可见性与操作权限' },
