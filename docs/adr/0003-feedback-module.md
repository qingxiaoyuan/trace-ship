...
反馈初始版本不做状态流转，仅作为意见收集与展示渠道；处理闭环通过在本模型上扩展状态字段实现，而非新建模块。

### 状态扩展（2026-07-30 补充）

- `Feedback` 新增 `status`（`open` 待处理 / `processed` 已处理）、`processed_by`、`processed_at` 字段（迁移 `0002`）。
- `POST /api/feedback/{id}/process/` 标记已处理，仅超管可操作，重复处理返回 40001；状态经序列化器 `read_only_fields` 保护，只能经该 action 变更。
- 前端每条反馈展示状态标签，超管对待处理项可见「设为已处理」按钮，已处理展示处理人与处理时间。
