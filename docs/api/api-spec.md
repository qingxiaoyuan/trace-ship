...
**响应：** 分页列表，含 `repo_count` / `member_count`。超管可查看全部产品；其他用户只返回具有显式成员记录的产品。`my_role` 为当前用户在项目中的有效角色（超管与项目负责人为 `manager`，软件管理员成员为 `software_admin`，非成员为 `null`），供前端按角色过滤项目下拉（如打包配置归属项目）。
...
- 超管可查看全部仓库；其他用户只可查看本人创建的仓库，以及已关联到本人所属产品的仓库。`repository.manage` 只授予维护能力，不扩大数据可见范围。
